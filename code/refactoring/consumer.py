...
class Consumer:
    ...
    def income(self, McountryBank, McountryCentralBank):
        ...
        if (
            self.depositIncome < -0.0001
            or self.DividendShare < -0.0001
            or self.capitalDismiss < -0.0001
            or self.laborIncome < -0.0001
    ...
    def consumptionDemand(
        self, McountryBank, McountryCentralBank, DcountryFailureProbability
    ):
        self.PastInvesting = self.Investing
        self.PastDepositing = self.Depositing
        if self.PastInvesting < -0.0001 or self.Depositing < -0.00001:
            print("stop", stop)
        self.Depositing = self.Depositing + self.y
        self.PastDepositing = self.Depositing
        self.PastDepositAllocated = self.depositAllocated
        pastTotalWealth = (
            self.Investing + self.Depositing + self.A + self.redistribution
        )
        ...
        self.savingIncome = 0
        desiredConsumption = (
            self.cDisposableIncome * self.disposableIncome
            + self.cWealth * self.Depositing
        )
        ...
        if (
            self.A + self.Investing < self.desiredA - 0.001
            or self.A + self.Investing > self.desiredA + 0.001
        ):
            print("stop", stop)
        if (
            self.totalWealth < pastTotalWealth - 0.001
            or self.totalWealth > pastTotalWealth + 0.001
        ):
            print("stop", stop)
        if (
            self.A < -0.000001
            or self.Investing < -0.0000001
            or self.Depositing < -0.000001
            or self.desiredA < -0.000001
            or self.reducingA < -0.000001
        ):
            print("stop", stop)
    ...
    def computeLiquidityPreference(self, DcountryFailureProbability):
        ...
        if (
            self.profitRate * (1 - DcountryFailureProbability[self.country])
            - self.rDeposit
        ) > 0 and self.A > 0.0:
            self.liqPref = liq * math.exp(
                -1.0
                * (
                    self.profitRate * (1 - DcountryFailureProbability[self.country])
                    - self.rDeposit
    ...
    def depositAllocation(self, McountryBank, McountryCentralBank):
        ...
        if (
            self.DepositToCheck < self.Deposit - 0.01
            or self.DepositToCheck > self.Deposit + 0.01
        ):
            print("stop", stop)
        ...
        if ideBank != self.country:
            McountryBank[self.country][ideBank].depositVariation(
                depositVariation, self.ide, McountryCentralBank
            )
        if ideBank == self.country:
            McountryCentralBank[self.country].depositVariation(
                depositVariation, self.ide
            )
    ...
    def ownershipCheck(self, McountryBank):
        ...
        for firm in self.DLA:
            ...
            if firm[0] == "B":
                ...
                if (
                    McountryBank[self.country][firm].Downer[self.ide][2]
                    > self.DLA[firm][2] + 0.000001
                    and McountryBank[self.country][firm].Downer[self.ide][2]
                    < self.DLA[firm][2] - 0.000001
                ):
                    print("stop", stop)
    ...
    def laborSupply(self, McountryUnemployement):
        u = McountryUnemployement[self.country]
        self.wageDirection = 0
        if self.l < self.ls:
            self.wP = random.uniform(
                self.wageDemanded, self.wageDemanded * (1 - self.delta)
            )
            self.wageDemanded = self.wP
        a = random.uniform(0, 1)
        if self.l >= self.ls and a < math.exp(-u * self.upsilon):
            Umin = self.wageDemanded
            Umax = self.wageDemanded * (1 + self.delta)
            self.wP = random.uniform(Umin, Umax)
            self.wageDemanded = self.wP
        if self.wageDemanded < self.wmin:
            self.wageDemanded = self.wmin
    ...
    def paying(self, payment, McountryBank, McountryCentralBank):
        ...
        for bank in LbankDeposit:
            ...
            if bank != self.country:
                McountryBank[self.country][bank].depositWithdrawal(
    ...
    def receiving(self, payment, McountryBank, McountryCentralBank):
        ...
        if ideBank != self.country:
            McountryBank[self.country][ideBank].depositInjection(
