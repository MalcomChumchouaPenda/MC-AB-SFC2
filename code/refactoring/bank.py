...
class Bank:
    ...
    def computeProbProvidingLoan(self, leverage):
    ...
    def existence(
        ...
        if self.closing == "no" or self.closing == "yes":
            ...
            for firm in self.Mloan:
                potentialEnter = (
                    potentialEnter + self.Mloan[firm][2] * self.Mloan[firm][3]
                )
            ...
            for agent in self.Mdeposit:
                if agent[0] == "C":
                    potentialExit = (
                        potentialExit
                        + self.Mdeposit[agent][2] * self.Mdeposit[agent][3]
                    )
                    self.potentialExitConsumer = (
                        self.potentialExitConsumer
                        + self.Mdeposit[agent][2] * self.Mdeposit[agent][3]
                    )
                if agent[0] == "F":
                    self.potentialExitFirm = (
                        self.potentialExitFirm
                        + self.Mdeposit[agent][2] * self.Mdeposit[agent][3]
                    )
            ...
            potentialExitCB = (
                self.loanDiscount * McountryCentralBank[self.country].rDiscount
            )
            ...
            if self.loanAllocated > self.PreviousA:
                self.depositNotUsed = self.depositReceived - (
                    self.loanAllocated - self.PreviousA
                )
            self.pastServiceFirm = self.serviceFirm
            self.serviceFirm = 0
            self.depositReimboursed = 0
        if (
            self.A > 1 * self.Fcost * DcountryAvWage[self.country]
            and self.A > 0.0
            and self.closing == "no"
        ):
        ...
        if (
            ...
            if self.A >= 0:
                ...
                self.depositReimboursed = (
                    self.depositReceived + self.potentialExitDeposit
                )
                self.repayingCBLoan(McountryCentralBank)
                self.loss = 0
            if self.A < 0:
    ...
    def capitalVariation(self, McountryConsumer, McountryBank, McountryCentralBank):
        ...
        for consumeride in self.Downer:
            if self.closing == "yes":  # or self.A<=0:
                ...
                ConsumerA = McountryConsumer[self.homecountry][consumeride].DLA[
                    self.ide
                ][2]
                ...
                McountryConsumer[self.homecountry][consumeride].receiving(
                    DismissShare, McountryBank, McountryCentralBank
                )
                ...
                ConsumerAshare = (
                    self.A
                    * McountryConsumer[self.homecountry][consumeride].DLA[self.ide][4]
                )
                subtotal = (
                    subtotal
                    + McountryConsumer[self.homecountry][consumeride].DLA[self.ide][4]
                ...
                checkPreviousA = (
                    checkPreviousA
                    + McountryConsumer[self.homecountry][consumeride].DLA[self.ide][2]
                )
                ...
                if (
                    ...
                    print("stop", stop)
                if (
                    self.Downer[consumeride][2]
                    < McountryConsumer[self.homecountry][consumeride].DLA[self.ide][2]
                    - 0.00000001
                    or self.Downer[consumeride][2]
                    > McountryConsumer[self.homecountry][consumeride].DLA[self.ide][2]
                    + 0.00000001
                ):
                    print("stop", stop)
                ...
                DismissShare = (
                    (self.capitalDismiss + self.dividends)
                    * ConsumerAshare
                    / float(self.A)
                )
                ...
                McountryConsumer[self.homecountry][consumeride].DLA[self.ide][
                    2
                ] = ConsumerAshare
                McountryConsumer[self.homecountry][consumeride].DLA[self.ide][
                    4
                ] = ratioA
                self.Downer[consumeride][2] = McountryConsumer[self.homecountry][
                    consumeride
                ].DLA[self.ide][2]
                self.Downer[consumeride][4] = McountryConsumer[self.homecountry][
                    consumeride
                ].DLA[self.ide][4]
                checkA = (
                    checkA
    ...
    def depositInjection(self, injection, ideAgent, McountryCentralBank):
        ...
        McountryCentralBank[self.country].Reserves = (
            McountryCentralBank[self.country].Reserves + injection
        )
    ...
    def depositWithdrawal(self, reduction, ideAgent, McountryCentralBank):
        ...
        McountryCentralBank[self.country].Reserves = (
            McountryCentralBank[self.country].Reserves - reduction
        )
    ...
    def repayingCBLoan(self, McountryCentralBank):
        ...
        McountryCentralBank[self.country].loanDiscount = (
            McountryCentralBank[self.country].loanDiscount - reduction
        )
        self.Reserves = self.Reserves - reduction
        potentialExitCB = (
            self.loanDiscount * McountryCentralBank[self.country].rDiscount
        ...
        McountryCentralBank[self.country].interestLoanDiscount = (
            McountryCentralBank[self.country].interestLoanDiscount + potentialExitCB
        )
        self.loanDiscount = 0
    ...
    def exitWithdrawal(self, reduction, McountryEtat, McountryCentralBank):
        if reduction <= self.Reserves:
            ...
            McountryCentralBank[self.country].Reserves = (
                McountryCentralBank[self.country].Reserves - self.Reserves
            )
            self.Reserves = 0
            McountryEtat[self.country].coveringDeposit(covering, McountryCentralBank)

    def loanCreation(
        self, firmIde, loanValue, interestRate, countryFirm, McountryCentralBank
    ):
        if self.country == countryFirm:
            self.loanCreationDomestic(firmIde, loanValue, interestRate, countryFirm)
        else:
            self.loanCreationForeigner(
                firmIde, loanValue, interestRate, countryFirm, McountryCentralBank
            )
    ...
    def buyingBondsDomestic(self, bondsVolume, rBonds, McountryCentralBank):
        ...
        McountryCentralBank[self.country].Reserves = (
            McountryCentralBank[self.country].Reserves - bondsVolume
        )
    ...
    def liquidatingBondsDomestic(self, rBonds, McountryCentralBank, McountryEtat):
        ...
        McountryCentralBank[self.country].Bonds = (
            McountryCentralBank[self.country].Bonds + bondsVolume
        )
        McountryEtat[self.country].Bonds = (
            McountryEtat[self.country].Bonds + bondsService
        )
        self.bondsInterest = self.Bonds * rBonds
        McountryEtat[self.country].interestExpenditure = (
            McountryEtat[self.country].interestExpenditure + self.bondsInterest
        )
        self.pastBonds = self.Bonds
        self.Bonds = 0
    ...
    def liquidatingBondsOpen(self, rBonds, McountryCentralBank, McountryEtat):
        ...
        for bonds in self.Mbonds:
            ...
            McountryEtat[countryEtat].Bonds = (
                McountryEtat[countryEtat].Bonds + bondsService
            )
            ...
            McountryCentralBank[countryEtat].Bonds = (
                McountryCentralBank[countryEtat].Bonds + bondsVolume
            )
            McountryEtat[countryEtat].interestExpenditure = (
                McountryEtat[countryEtat].interestExpenditure + bondsService
            )
            if self.country != countryEtat:
                McountryCentralBank[self.country].bondRepeymentInflow = (
                    McountryCentralBank[self.country].bondRepeymentInflow + bondsVolume
                )
                McountryCentralBank[countryEtat].bondRepeymentOutflow = (
                    McountryCentralBank[countryEtat].bondRepeymentOutflow + bondsVolume
                )
        self.pastBonds = self.Bonds
        self.Bonds = 0
    ...
    def askingLoanCentralBank(self, askingLoan, McountryCentralBank):
        ...
        McountryCentralBank[self.country].loanDiscount = (
            McountryCentralBank[self.country].loanDiscount + askingLoan
        )
        McountryCentralBank[self.country].Reserves = (
            McountryCentralBank[self.country].Reserves + askingLoan
        )
    ...
    def checkNetWorth(self):
        ...
        printingControl = "no"
        if (
            (Liabilities - Assets) / float(Liabilities + Assets) > 0.0001
            or (Liabilities - Assets) / float(Liabilities + Assets) < -0.0001
            or self.Reserves < -0.001
        ):
            print("stop", stop)
