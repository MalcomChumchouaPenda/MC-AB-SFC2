...
class Etat:
    ...
    def taxationConsumer(self, McountryConsumer, McountryBank, McountryCentralBank):
        ...
        for consumer in McountryConsumer[self.country]:
            ...
            if McountryConsumer[self.country][consumer].y > 0.0:
                ...
                self.taxCollectedImponibleConsumer = (
                    self.taxCollectedImponibleConsumer + taxable
                )
                taxVolume = self.taxRate * taxable
                self.taxCollectedConsumer = (
                    self.taxCollectedConsumer + self.taxRate * taxable
                )
                McountryConsumer[self.country][consumer].taxes = self.taxRate * taxable
                McountryConsumer[self.country][consumer].y = (
                    McountryConsumer[self.country][consumer].y - self.taxRate * taxable
                )
                McountryConsumer[self.country][consumer].paying(
                    taxVolume, McountryBank, McountryCentralBank
                )
                self.LiquidityEtat = self.LiquidityEtat + taxVolume
                McountryCentralBank[self.country].DepositEtatCentralBank = (
                    McountryCentralBank[self.country].DepositEtatCentralBank + taxVolume
                )
    ...
    def taxationFirm(self, McountryFirm, McountryBank, McountryCentralBank):
        ...
        for firm in McountryFirm[self.country]:
            if (
                McountryFirm[self.country][firm].profit > 0.0000000001
                and McountryFirm[self.country][firm].closing == "no"
            ):
                self.taxCollectedFirm = (
                    self.taxCollectedFirm
                    + self.taxRate * McountryFirm[self.country][firm].profit
                )
                McountryFirm[self.country][firm].netProfit = (
                    1 - self.taxRate
                ) * McountryFirm[self.country][firm].profit
                pastA = McountryFirm[self.country][firm].A
                McountryFirm[self.country][firm].A = (
                    McountryFirm[self.country][firm].A
                    - self.taxRate * McountryFirm[self.country][firm].profit
                )
                taxVolume = self.taxRate * McountryFirm[self.country][firm].profit
                McountryFirm[self.country][firm].paying(
                    taxVolume, McountryBank, McountryCentralBank
                )
                newA = McountryFirm[self.country][firm].A
                self.taxCollectedImponibleFirm = (
                    self.taxCollectedImponibleFirm
                    + McountryFirm[self.country][firm].profit
                )
                self.LiquidityEtat = self.LiquidityEtat + taxVolume
    ...
    def taxationBank(self, McountryBank, McountryCentralBank):
        ...
        for bank in McountryBank[self.country]:
            if (
                McountryBank[self.country][bank].profit > 0.0000000001
                and McountryBank[self.country][bank].closing == "no"
            ):
                self.taxCollectedBank = (
                    self.taxCollectedBank
                    + self.taxRate * McountryBank[self.country][bank].profit
                )
                McountryBank[self.country][bank].netProfit = (
                    1 - self.taxRate
                ) * McountryBank[self.country][bank].profit
                McountryBank[self.country][bank].A = (
                    McountryBank[self.country][bank].A
                    - self.taxRate * McountryBank[self.country][bank].profit
                )
                reduction = self.taxRate * McountryBank[self.country][bank].profit
                taxVolume = reduction
                McountryBank[self.country][bank].reserveWithdrawal(
                    reduction, McountryCentralBank
                )
                self.LiquidityEtat = self.LiquidityEtat + taxVolume
                self.taxCollectedImponibleBank = (
                    self.taxCollectedImponibleBank
                    + McountryBank[self.country][bank].profit
                )
                McountryCentralBank[self.country].DepositEtatCentralBank = (
    ...
    def expectingTaxation(
        self, McountryConsumer, McountryCentralBank, McountryUnemployement
    ):
        ...
        if McountryCentralBank[self.country].profit > 0:
            ...
            McountryCentralBank[self.country].pastProfitPos = McountryCentralBank[
                self.country
            ].profit
        self.expectedTaxCollected = 0
        for consumer in McountryConsumer[self.country]:
            self.expectedTaxCollected = (
                self.expectedTaxCollected
                + McountryConsumer[self.country][consumer].y * self.taxRate
            )
        self.expectedTaxCollected = (
            self.expectedTaxCollected
            + self.profitCB
    ...
    def effectiveTaxation(self):
        ...
        self.publicDeficit = (
            self.realG
            + self.pastInterestExpenditure
            - self.taxCollected
            + self.Salvatage
        )
        self.taxRate = self.followingTaxRate
        if (
            self.taxCollected > self.expectedTaxCollected + 0.001
            or self.taxCollected < self.expectedTaxCollected - 0.001
        ):
            print("stop", stop)

    def governmentPlannedExpenditure(
        self,
        McountryConsumer,
        McountryFirm,
        McountryCentralBank,
        ...
        if self.startingTime < self.endTransitionTime:
            self.realG = (
                McountryAvPrice[self.country] * DglobalPhi[self.country] * self.initialG
            )
            self.G = (
                McountryAvPrice[self.country] * DglobalPhi[self.country] * self.initialG
            )
        if self.startingTime >= self.endTransitionTime:
            ...
            desiredG = min(
                McountryAvPrice[self.country]
                * DglobalPhi[self.country]
                * self.initialG,
                self.gMax * McountryY[self.country],
            )
        ...
        if (
            self.LiquidityEtat
            > self.surplusEtat + self.taxCollectedFirm + self.taxCollectedBank + 0.001
            or self.LiquidityEtat
            < self.surplusEtat + self.taxCollectedFirm + self.taxCollectedBank - 0.001
        ):
            print("stop", stop)
        deficitSecondary = (
            self.bondsRepayments
            + self.realG
            - self.surplusEtat
            - self.expectedTaxCollected
            + self.profitCB
        ...
        if McountryY[self.country] > 0.01:
            self.rBonds = (
                self.xiBonds * (self.bondsSupply / float(McountryY[self.country]))
    ...
    def bondsToCentralBank(self, McountryCentralBank):
        if self.bondsSupply > self.bondsAllocated:
            ...
            McountryCentralBank[self.country].DepositEtatCentralBank = (
                McountryCentralBank[self.country].DepositEtatCentralBank + self.bondsCB
            )
            self.LiquidityEtat = (
                self.LiquidityEtat + self.bondsCB
            )  # McountryCentralBank[self.country].basicMoney
        self.Bonds = self.bondsSupply
        self.pastBonds = self.Bonds
        self.LiquidityEtat = (
            self.LiquidityEtat - self.bondsRepayments
        )  # paying back previous debt
        McountryCentralBank[self.country].DepositEtatCentralBank = (
            McountryCentralBank[self.country].DepositEtatCentralBank
            - self.bondsRepayments
        )

    def redistributionConsumer(
        self, McountryConsumer, McountryBank, McountryCentralBank, McountryAvPrice, t
    ):
        self.effectiveTaxation()
        if (
            self.LiquidityEtat < -0.000001
            or self.LiquidityEtat + self.expenditure < self.realG - 0.00001
        ):
            print("stop", stop)
        self.Redistribution = 0  # self.LiquidityEtat
        redistributionConsumer = (self.realG) / float(
            len(McountryConsumer[self.country])
        )
        for consumer in McountryConsumer[self.country]:
            McountryConsumer[self.country][
                consumer
            ].redistribution = redistributionConsumer
            self.Redistribution = (
                self.Redistribution
                + McountryConsumer[self.country][consumer].redistribution
            )
            payment = McountryConsumer[self.country][consumer].redistribution
            McountryConsumer[self.country][consumer].receiving(
                payment, McountryBank, McountryCentralBank
            )
        ...
        if self.LiquidityEtat < -0.001:
            print("stop", stop)

    def coveringDeposit(self, ammount, McountryCentralBank):
        McountryCentralBank[self.country].Bonds = (
            McountryCentralBank[self.country].Bonds + ammount
        )
