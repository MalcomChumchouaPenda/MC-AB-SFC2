...
class MatchingCredit:
        ...
        for country in McountryFirm:
            ...
            for firm in McountryFirm[country]:
                ...
                if McountryFirm[country][firm].loanDemand > 0.001:
                    leverage = McountryFirm[country][firm].loanDemand / float(
                    ...
                    loanDemand = [
                        ...
                        McountryFirm[country][firm].loanDemand,
                        0,
                        leverage,
                    ]
            ...
            for bank in McountryBank[country]:
                ...
                McountryBank[country][bank].loanSupply = (
                    McountryBank[country][bank].mu1 * McountryBank[country][bank].A
                )  # float('inf')
                McountryBank[country][bank].maxLoanFirm = McountryBank[country][
                    bank
                ].loanSupply
                ...
                if McountryBank[country][bank].loanSupply > 0.001:
                    loanSupply = [
                        ...
                        posBank,
                        McountryBank[country][bank].country,
                        McountryBank[country][bank].loanSupply,
                        0,
                        McountryBank[country][bank].maxLoanFirm,
                    ]
    ...
    def matchCreditOpen(self, McountryFirm, McountryBank, McountryCentralBank):
        ...
        while i < li:
            ...
            while j < lj:
                if (
                    self.MloanSupply[j][0]
                    in McountryFirm[countryFirm][ideFirm].Lcreditor
                ) == False:
                    ...
                    probLoan = McountryBank[countryBank][
                        ideBank
                    ].computeProbProvidingLoan(leverage)
                    ...
                    if loan > 0.001:
                        ...
                        if ideBank != McountryBank[countryBank][ideBank].ide:
                            print("stop", stop)
                        interestRate = McountryBank[countryBank][
                            ideBank
                        ].computeInterestRate(leverage)
                        interestRateDeposit = McountryBank[countryBank][
                            ideBank
                        ].rDeposit
                        McountryFirm[countryFirm][ideFirm].receavingLoan(
                            ideBank,
                            loan,
                            interestRate,
                            interestRateDeposit,
                        ...
                        if countryFirm != countryBank:
                            self.creditCapitalInflow[countryFirm] = (
                                self.creditCapitalInflow[countryFirm] + loan
                            )
                            self.creditCapitalOutflow[countryBank] = (
                                self.creditCapitalOutflow[countryBank] + loan
                            )
                            McountryCentralBank[countryFirm].moneyInflow = (
