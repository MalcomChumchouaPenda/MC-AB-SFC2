    ...
    def extractingBondsOpen(self, McountryBank, McountryEtat):
        ...
        for country in McountryEtat:
            ...
            for i in range(int(nTranches)):
                ...
                self.MbondsSupply.append(
                    [
                        country,
                        McountryEtat[country].bondsSupply / float(nTranches),
                        0,
                        leverage,
                        i,
                    ]
    ...
    def matchingBondsOpen(self, McountryBank, McountryCentralBank, McountryEtat):
        ...
        while i < li:
            ...
            while j < lj:
                ...
                probBond = McountryBank[countryBank][ideBank].computeProbBuyingBondLoan(
                    leverage
                )
                ...
                if bondExchange > 0.001:
                    ...
                    McountryBank[countryBank][ideBank].buyingBondsOpen(
                        bondExchange, rBonds, McountryCentralBank, countryEtat
                    )
                    McountryEtat[countryEtat].bondsAllocated = (
                        McountryEtat[countryEtat].bondsAllocated + bondExchange
                    )
                    McountryEtat[countryEtat].LiquidityEtat = (
                        McountryEtat[countryEtat].LiquidityEtat + bondExchange
                    )
                    ...
                    if countryBank != countryEtat:
                        self.creditBondInflow[countryEtat] = (
                            self.creditBondInflow[countryEtat] + bondExchange
                        )
                        self.creditBondOutflow[countryBank] = (
                            self.creditBondOutflow[countryBank] + bondExchange
                        )
                        McountryCentralBank[countryEtat].bondRepeymentInflow = (
                            McountryCentralBank[countryEtat].bondRepeymentInflow
                            + bondExchange
                        )
                        McountryCentralBank[countryBank].bondRepeymentOutflow = (
                            McountryCentralBank[countryBank].bondRepeymentOutflow
                            + bondExchange
                        )
