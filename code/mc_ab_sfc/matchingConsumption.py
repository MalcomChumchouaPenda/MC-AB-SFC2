...
class MatchingConsumption:
    ...
    def consuming(
        ...
        self.consumptionExtract(
            McountryFirm, McountryConsumer, McountryEtat, McountryAvPrice, t
        )
        if self.propTradable > 0.001:
            self.consumptionConsumer5(
                McountryConsumer,
                McountryFirm,
                t,
                McountryEtat,
        ...
        if 1 - self.propTradable > 0.001:
            self.consumptionConsumer5NotTradable(
                McountryConsumer,
                McountryFirm,
                t,
                McountryEtat,
    ...
    def consumptionExtract(
        self, McountryFirm, McountryConsumer, McountryEtat, McountryAvPrice, time
    ):
        self.Lsale = []
        ...
        for country in self.Lcountry:
            ...
            for firm in McountryFirm[country]:
                ...
                xProd = (
                    McountryFirm[country][firm].productionEffective
                    + McountryFirm[country][firm].inventory
                )
                size = (
                ...
                if (
                    xProd > 0
                    and McountryFirm[country][firm].closing == "no"
                    and McountryFirm[country][firm].tradable == "yes"
                ):
                ...
                if (
                    xProd > 0
                    and McountryFirm[country][firm].closing == "no"
                    and McountryFirm[country][firm].tradable == "no"
                ):
                    self.DcountrySaleNotTradable[country].append(
                        [p, xProd, ide, firmcountry, 0, pos, omega]
                    )
                    pos = pos + 1
        self.Lconsuming = []
        ...
        for country in McountryConsumer:
            ...
            for consumer in McountryConsumer[country]:
                ...
                for consuPast in McountryConsumer[country][consumer].LConsumptionEff:
                    McountryConsumer[country][consumer].pastLConsumptionEff.append(
                        consuPast[2]
                    )
    ...
    def consumptionConsumer5(
        ...
        for Lconsumer in self.Lconsuming:
            ...
            country = Lconsumer[1]
            consumption = McountryConsumer[country][consumer].Consumption * (
                self.propTradable
            )  ####
            ...
            for good in LchoiceGood:
                ...
                if x > 0 and consumption > 0:
                    ...
                    u = McountryConsumer[country][consumer].ordinating(
                        p, firmide, omega, avPrice
                    )  ###
            ...
            if len(LconsumptionU) > 0:
                ...
                while j < lj:
                    ...
                    McountryConsumer[country][consumer].LConsumptionEff.append(
                        [p, quantityConsumed, firmide, firmcountry]
                    )
                    ...
                    if firmcountry != country:
                        McountryConsumer[country][consumer].Import = (
                            McountryConsumer[country][consumer].Import
                            + quantityConsumed * p
                        )
                    ...
                    McountryFirm[firmcountry][firmide].xSold = (
                        McountryFirm[firmcountry][firmide].xSold + quantityConsumed
                    )
                    ...
                    if consumption > 0:
                        j = j + 1
                shift = 0
                McountryConsumer[country][consumer].paying(
                    totalPayment, McountryBank, McountryCentralBank
                )
                LdelSale.sort()
    ...
    def consumptionConsumer5NotTradable(
        self,
        McountryConsumer,
        McountryFirm,
        t,
            ...
            for Lconsumer in self.DconsumingNotTradable[country]:
                ...
                if len(LconsumptionU) > 0:
                    ...
                    while j < lj:
                        ...
                        self.DcountrySaleNotTradable[country][position][1] = (
                            self.DcountrySaleNotTradable[country][position][1]
                            - quantityConsumed
                        )
                        self.DcountrySaleNotTradable[country][position][4] = (
                            self.DcountrySaleNotTradable[country][position][4]
                            + quantityConsumed
                        )
                        McountryFirm[firmcountry][firmide].lebalance.Lsold[2] = (
