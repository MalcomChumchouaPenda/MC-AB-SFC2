    ...
    def debtExplotionStop(self, McountryY, DcumulativeDTBC, t):
        for country in self.Lcountry:
            if McountryY[country] > 0:
                if (
                    self.McountryEtat[country].Bonds / McountryY[country] > 10.0
                    and t > 100
                ):
                    print()
                    print()
                    print()
                    print(
                        "DEBT EXPLOTION        DEBT EXPLOTION           DEBT EXPLOTION             DEBT EXPLOTION           DEBT EXPLOTION"
                    )
                    print()
                    print(
                        "debt/GDP",
                        self.McountryEtat[country].Bonds / McountryY[country],
                    )
                    print("stop", stop)

    def debtExplotionBreak(self, McountryY, DcumulativeDTBC, t):
        self.breaking = "no"
        for country in self.Lcountry:
            if McountryY[country] > 0:
                if (
                    self.McountryEtat[country].Bonds / McountryY[country] > 5.0
                    and t > 100
                ):
                    print()
                    print()
