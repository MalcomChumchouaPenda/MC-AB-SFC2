...
class CentralBankUnion:
    ...
    def taylorRule(
        ...
        if t > self.endTransitionTime:
            ...
            rcB = (
                self.rBar * (1 - self.csi)
                + self.csi * self.rDiscount
                + (1 - self.csi) * self.csiDP * (inflation - self.inflationTarget)
            )
    ...
    def checknetWorth(self):
        if (
            ...
            or self.DepositCentralbankUnion < self.LoanCentralbankUnion - 0.00001
        ):
            print("stop", stop)
